...
# RUTA RAÍZ PARA VALIDAR EL SERVIDOR
@app.get("/")
def read_root():
    return {
        "mensaje": "FinTech Nova Secure API Practice está en línea",
        "endpoints": ["/vulnerable/users/{username}", "/secure/users/{username}"]
    }
...
def get_user_vulnerable(username: str):
    conn = get_db()
    cursor = conn.cursor()
    # NUNCA HACER ESTO: concatenación directa de datos en la consulta SQL
...
def get_user_secure(username: str):
    conn = get_db()
    cursor = conn.cursor()
    # FORMA SEGURA: parametrización de la consulta
